...
const MyCert = (props) => {
  ...
  const [pageSt, setPageSt] = useState(false);
	...
	const [jobNameDetail, setJobNameDetail] = useState('');
	...
	const getAuthList = async () => {
		...
		if(response.code == 200){
			response.data.map((item) => {
				if(item.title == '프로필 인증'){
					setAuthList(item.lists);
				}
			})			
		}
	}
	...
	const getAuthInfo = async () => {
		...
		const response = await APIs.send(sData);
		console.log('getAuthInfo ::: ', response);
		if(response.code == 200){
			response.data.map((item, index) => {
				if(item.pa_name == '직업 인증'){
					...
					if(item.mpa_idx){
						...
						if(item.auth_yn == 'n'){
							setSchoolName(item.mpa_info1);
							setSchoolMajor(item.mpa_info2);
						}
					}else{
						setSchoolApiRes({});
						setSchoolName('');
											...
											>
												...
												<View style={styles.badgeBtnRight}>
													{realJobFile.path ? (														
														...
														<>
															{jobApiRes.auth_yn == 'n' ? (
																<View style={styles.stateView2}><Text style={styles.stateViewText2}>반려</Text></View>		
															) : null}
